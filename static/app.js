...
function renderGraph(data) {
    ...
    if (sorted.length <= VIRTUAL_THRESHOLD) {
        ...
        // Use the sidebar panel's natural height (constrained by CSS)
        // but cap at a reasonable viewport fraction
    ...
    _fetchWithTimeout('/api/graph?' + new URLSearchParams({ dir: document.getElementById('dirInput').value, ...getFilterValues() }))
        .then(r => r.json()).then(d => {
            if (d.error) {
                ...
                // renderGraph → _cInitCy handles the spinner and reveals
                // the graph once the layout finishes.
                renderGraph(d);
    ...
    _fetchWithTimeout('/api/file?' + new URLSearchParams(_getFileParams(fileId)))
        ...
        .then(data => {
            if (data.error) {
                ...
                document.getElementById('previewCode').textContent = data.suggestion || data.error;
                return;
            }
        ...
        .catch(err => {
            document.getElementById('previewMeta').textContent = '';
            document.getElementById('previewCode').textContent = err.message && err.message.startsWith('Request timed out') ? err.message : 'Failed to load file.';
        });
}
...
function runSimulation() {
    ...
    .then(r => r.json())
    .then(data => {
        if (data.error) { showToast('Error: ' + (data.suggestion || data.error)); resultsEl.innerHTML = ''; return; }
...
function storyLoad() {
    ...
    .catch(function(err) {
        _handleApiError(err, 'Failed to generate story.');
